...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-200 ${mobileOpen ? 'bg-slate-950/98 border-b border-slate-800 shadow-2xl' : 'glass-nav'}`}>
      <div className="relative z-50 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden p-2.5 rounded-xl text-slate-200 hover:text-white bg-slate-900/80 border border-slate-800 active:scale-95 transition-all min-h-[44px] min-w-[44px] flex items-center justify-center"
        ...
        {mobileOpen && (
          <div className="md:hidden pb-4 pt-2 animate-fade-in-up">
            <div className="p-3.5 rounded-2xl bg-slate-900/98 border border-slate-800 shadow-2xl space-y-2">
              {session?.user && (
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-emerald-500/20 mb-1">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black bg-emerald-500 text-slate-950 shrink-0">
                      {(session.user.name?.[0] || session.user.email?.[0] || 'U').toUpperCase()}
                    </div>
              ...
              <Link
                href="/#trial"
                className="flex items-center justify-between px-3.5 py-3 text-sm font-bold rounded-xl text-slate-100 bg-slate-800/80 hover:bg-slate-800 border border-slate-700/60 min-h-[44px]"
                onClick={() => setMobileOpen(false)}
              >
                <span className="flex items-center gap-2">🌱 7D Fresh Nutrition Trial</span>
                <span className="text-xs px-2.5 py-1 rounded bg-amber-400 text-slate-950 font-black">₹451</span>
              </Link>
              <Link
                href="/calculator"
                className="flex items-center justify-between px-3.5 py-3 text-sm font-bold rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 min-h-[44px]"
                onClick={() => setMobileOpen(false)}
              >
              ...
              <Link
                href="/#how-it-works"
                className="block px-3.5 py-2.5 text-sm font-medium rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white min-h-[44px] flex items-center"
                onClick={() => setMobileOpen(false)}
              >
                How It Works
              </Link>
              ...
              {session?.user ? (
                <Link
                  href="/dashboard"
                  className="flex items-center gap-2.5 px-3.5 py-3 text-sm font-bold rounded-xl text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 min-h-[44px]"
                  onClick={() => setMobileOpen(false)}
                >
                ...
                <Link
                  href="/login"
                  className="flex items-center gap-2.5 px-3.5 py-3 text-sm font-medium rounded-xl text-slate-200 hover:bg-slate-800 hover:text-emerald-400 border border-slate-800 min-h-[44px]"
                  onClick={() => setMobileOpen(false)}
                >
              ...
              <Link
                href="/#trial"
                className="w-full mt-2 py-3 text-sm font-black text-center rounded-xl text-slate-950 bg-emerald-500 hover:bg-emerald-400 block shadow-lg shadow-emerald-500/20 min-h-[44px] flex items-center justify-center"
                onClick={() => setMobileOpen(false)}
              >
                Claim 7D Trial (₹451) →
              </Link>
      ...
      {mobileOpen && (
        <div
          className="fixed inset-0 top-16 bg-slate-950/60 backdrop-blur-sm z-30 md:hidden animate-fade-in"
          onClick={() => setMobileOpen(false)}
          aria-hidden="true"
        />
      )}
