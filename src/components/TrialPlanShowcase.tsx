...
export default function TrialPlanShowcase({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3.5 pb-2">
        {JUST_BLOOMED_BOXES.map((box) => {
          ...
          return (
            <div
              key={box.day}
              className="glow-card flex flex-col justify-between rounded-2xl p-4 backdrop-blur-md bg-slate-900/90 border border-slate-800 transition-all duration-300 hover:border-slate-700 hover:-translate-y-1 relative overflow-hidden group shadow-lg"
            >
              {/* Day 6/7 Double Drop Tag */}
              {isDoubleDrop && (
                <div className="absolute top-0 right-0 px-2 py-0.5 rounded-bl-xl text-[9px] font-black uppercase tracking-wider bg-emerald-500 text-slate-950">
              ...
              <div>
                ...
                {/* Daily Goal Title */}
                <h4 className="text-xs font-black uppercase tracking-wider text-slate-100 mb-1">
                  {box.goal}
                </h4>
      ...
      <div className="mt-6 rounded-2xl p-5 sm:p-6 backdrop-blur-xl bg-slate-950/70 border border-emerald-500/30 flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Logistics Pill & Explanation */}
        <div className="flex items-start gap-3">
          <div className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center bg-emerald-500/20 text-emerald-400 text-lg border border-emerald-500/30">
            🚚
          </div>
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-amber-500/20 text-amber-400 border border-amber-500/30 mb-1">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-slate-800 pt-3 md:pt-0">
          <div className="text-left md:text-right">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-black text-emerald-400 font-mono">₹451</span>
              <span className="text-xs text-slate-400 font-medium">/ 7 boxes</span>
            </div>
          </div>
          ...
          <button
            ...
            className="px-6 py-3 rounded-xl text-xs sm:text-sm font-black text-slate-950 bg-emerald-500 hover:bg-emerald-400 transition-all shadow-lg shadow-emerald-500/20 hover:scale-105 active:scale-95 cursor-pointer whitespace-nowrap"
          >
            Claim 7D Trial →
          </button>
        </div>
